...
public  static  void main (String[] args) {
	...
	while( true ) {
	    ...
	    String password = keyboard.readLine();
            String strSql = "SELECT * FROM TUsers WHERE Login=? AND Password=?";
            ...
            if ( res.next() ) {
		strSql = "UPDATE TUsers SET ConnectionNbr=ConnectionNbr+1 WHERE ID=?";
		PreparedStatement stUpdate = conn.prepareStatement(strSql);
                stUpdate.setInt(1, res.getInt("ID"));
